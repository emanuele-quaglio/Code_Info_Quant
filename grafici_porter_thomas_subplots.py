# -*- coding: utf-8 -*-
"""
Created on Sun Feb 19 19:43:03 2023
...
def vectorizedfloat(x):
    for i in range(len(x)):
        x[i]=float(x[i])
    return x
def PORTERTHOMAS(filename, freqs="teo", ax=None, instance=None, sublabel=None):
    data=[]
    with open(filename, 'r') as file:
        ...
        while True:
            ...
            for mybins in [11]: #range(2,30,1): 
                ax.set_yscale('log')
                n, bins, patchess=ax.hist(line, bins=mybins, density=True, edgecolor='gray', color='lightgray')
                plt.plot()
                x=np.linspace(0, bins[-1], 100)
                #plt.plot(x, PT_distr(x, len(line),  bins, normalized=False), label='Porter-Thomas')
                ax.plot(x, PT_distr(x, len(line),  bins, normalized=True), label='Porter-Thomas', color='blue', linestyle='--')
                ...
                ax.plot(x, np.exp(lin_distr(x, *popt)),  label=fitlabel, color='black')
                #print(popt)
                ...
                ax.set_title('('+sublabel+')', loc='right', y=0.85)
...
fig,axes=plt.subplots(4,3, figsize=(6.7,10.12))
...
for i in [0,1,2,3]:
    for j in [0,1,2]:
        if (i,j)!=(3,1):
            PORTERTHOMAS("AT_q"+types[i]+"_vs_l.txt", freqs="teo", ax=axes[i][j], instance=inst[j], sublabel=labels[3*i+j])
        else:
           PORTERTHOMAS("AT_q"+types[i]+"_vs_l.txt", freqs="teo", ax=axes[i][j], instance='4', sublabel=labels[3*i+j]) 
for i in [0,1,2,3]:
    axes[i][0].set_ylabel('h', fontsize=12)
for j in [0,1,2]:
    axes[3][j].set_xlabel('q', fontsize=12) 
fig.tight_layout()
fig.savefig('PT_multiplot.pdf')
plt.show()