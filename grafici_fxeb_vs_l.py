...
def FXEBvsMplot(data, ftypes, general_label='', myax=plt.gca()):
    ...
    #plt.show()
def FXEBvsLplot(data, ftypes, general_label=''):
    #plt.xscale('log')
    plt.yscale('log')
    #plt.ylim(ymin=0.2)
    if 'sflin' in data[0]:
        plt.errorbar([row['l'] for row in data], [row['avflin'] for row in data], yerr=[row['sflin'] for row in data], fmt='.')
    ...
    plt.ylabel('$f_{XEB, lin}$', fontsize=14)
    
    #plt.minorticks_on()
    plt.tick_params(which='minor', labelcolor='white', labelsize=0)
...
'''
mydata, myftypes, gl=data_organizer("AT_q8_vs_l.txt", 'flin', 'flintt', 'flog', 'flogtt',
                                    ', linear chain')
#FXEBvsLplot(data_averager(mydata,myftypes ),myftypes, gl)
#FXEBvsLplot(mydata,myftypes, gl)

md=data_averager(mydata,myftypes )

print(md[-1]['avflin'], md[-1]['sflin'] )
#print(sum([row['avflin'] for row in md])/len(md))
#print(sum([row['sflin'] for row in md])/len(md))
c_start=9
print(min([row['avflin'] for row in md][c_start-2:]),max([row['avflin'] for row in md][c_start-2:]))
print(min([row['sflin'] for row in md][c_start-2:]),max([row['sflin'] for row in md][c_start-2:]))
'''
