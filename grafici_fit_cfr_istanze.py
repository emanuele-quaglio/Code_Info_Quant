...
def analisi_distr_freqs_theo(filename, distr, f_x=lambda x:x, f_y=lambda y:y, my_ylabel='frequenze teoriche'):
    ...
    with open(filename, 'r') as file:
        ...
        while True:
            ...
            s_param_list.append(np.sqrt(np.diag(pcov)))
...
#MAIN
#salvo a, b e sigma_a, sigma_b delle varie istanze del circuito
param_list=[]
s_param_list=[]
for i in range(10):
    analisi_distr_freqs_theo("analisi_distr_freqs_theo_"+str(i)+".txt", lin_distr, trim_edges, lambda x:np.log(trim_edges(x)), my_ylabel='frequenze teoriche (logaritmo)')
print ('lunghezza lista: ', len(s_param_list), 'lista: ', s_param_list )
plt.title('pendenze fit log frequenze teoriche, q=8, l=27')
plt.xlabel('# istanza')
plt.ylabel('pendenza')
plt.errorbar(range(len(param_list)),[param_list[i][0] for i in range(len(param_list))], yerr=[s_param_list[i][0] for i in range(len(s_param_list))], fmt='.')
plt.show()
plt.title('intercette fit log frequenze teoriche, q=8, l=27')
plt.xlabel('# istanza')
plt.ylabel('intercetta')
plt.errorbar(range(len(param_list)), [param_list[i][1] for i in range(len(param_list))], yerr=[s_param_list[i][1] for i in range(len(s_param_list))], fmt='.')
plt.show()    
